...
All three reliability components support pluggable storage backends:

- **RequestJournalStore** — custom journal backend (default: file-based JSONL)
- **IdempotencyRepository** — custom idempotency backend (default: file-based)
- **QueueStorage** — custom queue backend (default: file-based directory structure)

Example SQLite backend (see `RELIABILITY_STORAGE.md`):

```go
type SQLiteJournalStore struct { ... }
func (s *SQLiteJournalStore) Append(entry []byte) error { ... }
func (s *SQLiteJournalStore) Close() error { ... }

Reliability: fh.ReliabilityConfig{
    JournalStore: &SQLiteJournalStore{db},
}
```
